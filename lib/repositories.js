...
// use to make sure we return each repo in the array once
function repoFilter(value, index, self) {
...
function all(cached) {
  ...
  return Promise.all([ w3c(), whatwg() ])
   ...
   .then(data => data.flat().filter(repoFilter)) //make sure entries are uniq
   .then(data => {
...
if (config.debug) {
  Promise.all([ w3c(), whatwg() ])
   .then(data => data.flat().filter(repoFilter))
   .then(data => {
      extra().then(extras => {
        extras.forEach(extra => {
          if (data.find(r => r === extra)) monitor.log(`Remove ${extra} from extra`);
        })
      });
  }).catch(monitor.error);
}
