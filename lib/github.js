...
class Repository {
  ...
  async getTeams(ttl) {
    ...
    return octokit.get(`/v3/repos/${this.full_name}/teams`, param);
  }
  ...
  async getHooks(ttl) {
    ...
    if (ttl === undefined && this._labels) return this._labels;
    if (ttl !== undefined) param.ttl = ttl;
    return octokit.get(`/v3/repos/${this.full_name}/hooks`, param);
  }
  ...
  // labels is a string or an array of string
  ...
  // label is an objet with a name property (string)
  async removeIssueLabel(issue, label) {
    return octokit.request(`DELETE /repos/${this.full_name}/issues/${issue.number}/labels/{name}`, {
      name: label.name
    });
  }
