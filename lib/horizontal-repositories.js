...
const HR_LABELS_URL = "https://w3c.github.io/common-labels.json";
...
class HorizontalLabels {
  ...
  async checkRepositoryForLabels(repo) {
    ...
    const repo_labels = await repo.getLabels().catch(e => repo.getLabels(-1));
  ...
  async checkRepositoryForTeam(repo) {
    let teams = await repo.getTeams().catch(err => repo.getTeams(-1));
    const triage = teams.find(team => team.name == "horizontal-admin");
    if (!triage) {
      return setHRTeam(repo);
    } else {
      if (!triage.permissions.triage) {
        setHRTeam(repo).then(res => {
          console.log(res);
          monitor.log(`${repo.full_name} : triage team permission updated`);
        }).catch(err => {
          monitor.error(`${repo.full_name} : can't update triage team permission ${err}`);
        })
      }
    }
    return triage;
